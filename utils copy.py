...
# Search if there is a LIKE in da query and we cast it
        ...
        return {**state, "data": result, "answer": state.get("answer", []) + [sql_query], "used_tools": state.get("used_tools", []) + ["lookup_sales_data"]}
    except Exception as e:
        return {**state, "error": f"Error accessing data: {str(e)}"}
...
def parallel_analyze_data(state: State) -> State:
    ...
    sub_states = [
        {"prompt": state.get("prompt"), "data": state["data"], "tool_choice": state.get("tool_choice"), "answer": state.get("answer"), "analyze_data": state.get("analyze_data"), "used_tools": state.get("used_tools", []), "emphasis": sub_prompt}
    ...
    return {
        **state,
        ...
        "answer": state.get("answer", []) + [final_summary],
