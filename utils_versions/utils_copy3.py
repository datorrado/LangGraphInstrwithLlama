...
def generate_sql_query (state:State): 
    ...
    utils.start()
    tracker = EmissionsTracker(project_name=state["query_id"], measure_power_secs=1, log_level="critical", output_file=f"emissions_{state['id']}.csv", output_dir="3node")
    tracker.start()
    ...
    try:
        ...
        return {"sql_queries": sql_query, "energy_query": emissions, "query_id": state["query_id"], "cpu_query": utils.cpu_median, "gpu_query": utils.gpu_median}
...
def analyzing_data(state: State) -> State:
    ...
    tracker = EmissionsTracker(project_name=state["analysis_id"], measure_power_secs=1, log_level="critical", output_file=f"emissions_{state['id']}.csv", output_dir="3node")
    ...
    try:    
        ...
        "chart_config": state.get("chart_config"), "used_tools": state.get("used_tools", []) + ["analyzing_data"], "id": state.get("id"), "energy_analysis": tracker.stop(), "analysis_id": state.get("analysis_id"), "cpu_analysis": utils.cpu_median, "gpu_analysis": utils.gpu_median
        }
    except Exception as e:
        return {**state, "error": f"Error accessing data: {str(e)}"}
...
def create_visualization(state: State) -> State:
    ...
    utils = CPU_GPU_Tracker()
    utils.start()
    tracker = EmissionsTracker(project_name=state["ids_create_visualization"], measure_power_secs=1, log_level="critical", output_file=f"emissions_{state['id']}.csv", output_dir="3node")
    tracker.start()
    try:
...
def decide_tool(state: State, llm) -> State:
    ...
    tracker = EmissionsTracker(project_name=tool_id, measure_power_secs=1, log_level="critical",output_file=f"emissions_{state['id']}.csv", output_dir="3node")
    tracker.start()
    used_tools= state.get("used_tools", [])
    ...
    state["cpu_utilization_decide_tool"].append(utils.cpu_median)
    state["gpu_utilization_decide_tool"].append(utils.gpu_median)
...
def log_evaluation_to_csv(
    ...
    tool_name: str,
    id: str,
    file_path: str = "tool_evaluations_3.csv",
    energy=None,
    ...
    if isinstance(id_tool, list) and len(id_tool) == len(eval_df):
        for i, tool_id in enumerate(id_tool):
            emissions_file = Path("3node") / f"emissions_{id}.csv"
    ...
    # === Caso único: solo un id_tool ===
        ...
        emissions_file = Path(f"emissions_{id}.csv")
        if emissions_file.exists():
            try:
                df = pd.read_csv(emissions_file)
