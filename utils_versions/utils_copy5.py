...
def generate_sql_query (state:State): 
    ...
    utils.start()
    tracker = EmissionsTracker(project_name="lookup_sales_data",experiment_id=state["query_id"], measure_power_secs=1, log_level="critical", output_file=f"emissions_{state['id']}_{state['query_id']}.csv", output_dir="5node")
    tracker.start()
...
def analyzing_data(state: State) -> State:
    ...
    tracker = EmissionsTracker(project_name="analyzing_data", experiment_id=state["analysis_id"], measure_power_secs=1, log_level="critical", output_file=f"emissions_{state['id']}_{state['analysis_id']}.csv", output_dir="5node")
...
def create_visualization(state: State) -> State:
    ...
    utils = CPU_GPU_Tracker( )
    utils.start()
    tracker = EmissionsTracker(project_name="create_visualization",experiment_id=state["ids_create_visualization"], measure_power_secs=1, log_level="critical", output_file=f"emissions_{state['id']}_{state['ids_create_visualization']}.csv", output_dir="5node")
    tracker.start()
    try:
...
def decide_tool(state: State, llm) -> State:
    ...
    tracker = EmissionsTracker(project_name="decide_tool", experiment_id=tool_id, measure_power_secs=1, log_level="critical",output_file=f"emissions_{state['id']}_{tool_id}.csv", output_dir="5node")
    tracker.start()
    used_tools= state.get("used_tools", [])
