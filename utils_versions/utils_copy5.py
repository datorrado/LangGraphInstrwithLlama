...
def generate_sql_query (state:State): 
    ...
    utils.start()
    tracker = EmissionsTracker(project_name=state["query_id"], measure_power_secs=1, log_level="critical", output_file=f"emissions_{state['id']}.csv", output_dir="5node")
    tracker.start()
...
def analyzing_data(state: State) -> State:
    ...
    tracker = EmissionsTracker(project_name=state["analysis_id"], measure_power_secs=1, log_level="critical", output_file=f"emissions_{state['id']}.csv", output_dir="5node")
...
def create_visualization(state: State) -> State:
    ...
    utils = CPU_GPU_Tracker( )
    utils.start()
    tracker = EmissionsTracker(project_name=state["ids_create_visualization"], measure_power_secs=1, log_level="critical", output_file=f"emissions_{state['id']}.csv", output_dir="5node")
    tracker.start()
    try:
...
def decide_tool(state: State, llm) -> State:
    ...
    tracker = EmissionsTracker(project_name=tool_id, measure_power_secs=1, log_level="critical",output_file=f"emissions_{state['id']}.csv", output_dir="5node")
    tracker.start()
    used_tools= state.get("used_tools", [])
...
def log_evaluation_to_csv(
    ...
    if isinstance(id_tool, list) and len(id_tool) == len(eval_df):
        for i, tool_id in enumerate(id_tool):
            emissions_file = Path("5node") / f"emissions_{id}.csv"
